...
public class QuartzServiceImpl implements QuartzService {
     ...
     * @param jobTime 时间表达式 （这是没隔几秒运行一次）
     * @param jobTimes 运行的次数（<0 表示无数）
     * @param jobData 参数
     */
    @Override
            ...
            if (jobTimes < 0){
                ...
                trigger = TriggerBuilder
                        .newTrigger().withIdentity(jobName, jobGroupName).withSchedule(SimpleScheduleBuilder
                                .repeatSecondlyForever(1).withIntervalInSeconds(jobTime).withRepeatCount(jobTimes)).startNow().build();
            }
